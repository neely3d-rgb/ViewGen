...
#define LOCTEXT_NAMESPACE "SUE_GenPanel"

void SUE_GenPanel::Construct(const FArguments& InArgs)
{
	...
	// Bind HTTP callbacks
	HttpClient->OnComplete.BindRaw(this, &SUE_GenPanel::OnGenerationComplete);
	HttpClient->OnProgress.BindRaw(this, &SUE_GenPanel::OnGenerationProgress);
	HttpClient->OnError.BindRaw(this, &SUE_GenPanel::OnGenerationError);
	...
	HttpClient->OnSegmentationComplete.BindRaw(this, &SUE_GenPanel::OnSAM3SegmentationComplete);
					...
					[
						...
						[
							...
							[
								...
								.OnClicked(this, &SUE_GenPanel::OnGenerateFromGraphClicked)
							...
							[
								...
								.OnClicked(this, &SUE_GenPanel::OnSaveGraphClicked)
								.IsEnabled_Lambda([this]()
								{
									return GraphEditor.IsValid() && GraphEditor->GetNodes().Num() > 0;
								})
							...
							[
								...
								.OnClicked(this, &SUE_GenPanel::OnSaveGraphAsClicked)
								.IsEnabled_Lambda([this]()
								{
									return GraphEditor.IsValid() && GraphEditor->GetNodes().Num() > 0;
								})
							...
							[
								...
								.OnClicked(this, &SUE_GenPanel::OnLoadGraphClicked)
							]
							...
							[
								...
								.OnClicked(this, &SUE_GenPanel::OnGraphEditorFullscreen)
							]
						]
					]
				]
	...
	PostGCDelegateHandle = FCoreUObjectDelegates::GetPostGarbageCollect().AddRaw(this, &SUE_GenPanel::OnPostGarbageCollect);
}

SUE_GenPanel::~SUE_GenPanel()
{
...
void SUE_GenPanel::Tick(const FGeometry& AllottedGeometry, const double InCurrentTime, const float InDeltaTime)
{
...
void SUE_GenPanel::OnPostGarbageCollect()
{
...
TSharedRef<SWidget> SUE_GenPanel::BuildToolbar()
{
	return SNew(SHorizontalBox)
		...
		[
			...
			.OnClicked(this, &SUE_GenPanel::OnCaptureViewportClicked)
		...
		[
			...
			.OnClicked(this, &SUE_GenPanel::OnGenerateClicked)
			.IsEnabled_Lambda([this]()
			{
		...
		[
			...
			.OnClicked(this, &SUE_GenPanel::OnCancelClicked)
			.IsEnabled_Lambda([this]() { return bIsGenerating; })
		]
		...
		[
			...
			.OnClicked(this, &SUE_GenPanel::OnOpenSettingsClicked)
		];
}
...
TSharedRef<SWidget> SUE_GenPanel::BuildThumbnailPanel()
{
...
TSharedRef<SWidget> SUE_GenPanel::BuildThumbnailCard(const FText& Label, TSharedPtr<FSlateBrush>& OutBrush)
{
...
TSharedRef<SWidget> SUE_GenPanel::BuildPreviewPanel()
{
	return SNew(SBorder)
		.BorderImage(FAppStyle::GetBrush("ToolPanel.DarkGroupBorder"))
		.Padding(4.0f)
...
TSharedRef<SWidget> SUE_GenPanel::BuildPromptPanel()
{
	const UGenAISettings* Settings = UGenAISettings::Get();
	...
	return SNew(SVerticalBox)
		...
		[
			...
			[
				...
				.AutoWrapText(true)
				.OnTextChanged(this, &SUE_GenPanel::OnPromptTextChanged)
		...
		[
			...
			[
				...
				.AutoWrapText(true)
				.OnTextChanged(this, &SUE_GenPanel::OnNegativePromptTextChanged)
				.OnTextCommitted_Lambda([](const FText& NewText, ETextCommit::Type)
				{
...
TSharedRef<SWidget> SUE_GenPanel::BuildLoRAPanel()
{
	return SNew(SVerticalBox)
		...
		[
			...
			[
				...
				.OnClicked(this, &SUE_GenPanel::OnAddLoRAClicked)
			]
		]
...
TSharedRef<SWidget> SUE_GenPanel::BuildStatusBar()
{
	return SNew(SHorizontalBox)
...
FReply SUE_GenPanel::OnCaptureViewportClicked()
{
	const UGenAISettings* Settings = UGenAISettings::Get();
...
FReply SUE_GenPanel::OnGenerateClicked()
{
	const UGenAISettings* Settings = UGenAISettings::Get();
	...
	if (Settings->bAutoCameraPrompt && ViewportCapture->HasCameraData())
	{
		...
		if (!CameraDesc.IsEmpty())
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Camera prompt injected: %s"), *CameraDesc);
		}
	}
...
FReply SUE_GenPanel::OnCancelClicked()
{
	HttpClient->CancelRequest();
	bIsGenerating = false;
	CurrentProgress = 0.0f;
...
FReply SUE_GenPanel::OnAddLoRAClicked()
{
	FString Path;
	if (LoRAPathCombo.IsValid())
	{
	...
	for (int32 i = 0; i < UILoRAEntries.Num(); i++)
	{
		...
		[
			...
			[
				...
				.OnClicked(this, &SUE_GenPanel::OnRemoveLoRA, Index)
			]
		];
	}
...
FReply SUE_GenPanel::OnRemoveLoRA(int32 Index)
{
	if (UILoRAEntries.IsValidIndex(Index))
	{
		...
		for (int32 i = 0; i < UILoRAEntries.Num(); i++)
		{
			...
			[
				...
				[
					...
					.OnClicked(this, &SUE_GenPanel::OnRemoveLoRA, Idx)
				]
			];
		}
...
FReply SUE_GenPanel::OnOpenSettingsClicked()
{
...
TSharedRef<SWidget> SUE_GenPanel::MakeSettingsRow(const FText& Label, TSharedRef<SWidget> ValueWidget)
{
	return SNew(SHorizontalBox)
...
void SUE_GenPanel::InitSamplerSchedulerOptions()
{
	const UGenAISettings* Settings = UGenAISettings::Get();
...
TSharedPtr<FString> SUE_GenPanel::FindOrAddOption(TArray<TSharedPtr<FString>>& Options, const FString& Value)
{
...
void SUE_GenPanel::FetchAndPopulateModels()
{
	if (!HttpClient.IsValid()) return;

	HttpClient->OnModelListsFetched.BindRaw(this, &SUE_GenPanel::OnModelsReceived);
	HttpClient->FetchAvailableModels();
}

SUE_GenPanel::EModelArch SUE_GenPanel::DetectArchitecture(const FString& ModelName)
{
...
bool SUE_GenPanel::IsCompatibleWithMode(const FString& ModelName, bool bFluxMode)
{
...
void SUE_GenPanel::FilterModelsForCurrentMode()
{
	const UGenAISettings* Settings = UGenAISettings::Get();
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Filtered for %s — showing %d LoRAs, %d ControlNets (from %d / %d total)"),
		bFluxMode ? TEXT("Flux") : TEXT("SD"),
		LoRAModelOptions.Num(), ControlNetOptions.Num(),
		AllLoRAs.Num(), AllControlNets.Num());
}

void SUE_GenPanel::OnModelsReceived(const TArray<FString>& Checkpoints, const TArray<FString>& LoRAs, const TArray<FString>& ControlNets, const FGeminiNodeOptions& GeminiOptions, const FKlingNodeOptions& KlingOptions)
{
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Model dropdowns updated — %d checkpoints, %d LoRAs, %d ControlNets, %d Gemini models"),
		AllCheckpoints.Num(), AllLoRAs.Num(), AllControlNets.Num(), GeminiModelOptions.Num());
}

void SUE_GenPanel::PopulateGeminiDropdowns(const FGeminiNodeOptions& Options)
{
	const UGenAISettings* Settings = UGenAISettings::Get();
...
void SUE_GenPanel::PopulateKlingDropdowns(const FKlingNodeOptions& Options)
{
	const UGenAISettings* Settings = UGenAISettings::Get();
...
TSharedRef<SWidget> SUE_GenPanel::BuildCollapsibleSection(const FText& Title, bool& bExpandedFlag, TSharedRef<SWidget> Content)
{
	return SNew(SVerticalBox)
...
TSharedRef<SWidget> SUE_GenPanel::BuildSettingsPanel()
{
...
void SUE_GenPanel::ResetSettingsToDefaults()
{
	UGenAISettings* Settings = UGenAISettings::Get();
	if (!Settings) return;
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Settings reset to defaults"));
}

void SUE_GenPanel::RefreshUIFromSettings()
{
	const UGenAISettings* Settings = UGenAISettings::Get();
	if (!Settings) return;
...
void SUE_GenPanel::RefreshPresetList()
{
	PresetOptions.Empty();
...
void SUE_GenPanel::SaveCurrentPreset()
{
...
void SUE_GenPanel::LoadSelectedPreset()
{
...
void SUE_GenPanel::DeleteSelectedPreset()
{
	if (!SelectedPreset.IsValid() || SelectedPreset->IsEmpty()) return;
...
void SUE_GenPanel::ApplyAdherenceToSettings(float Adherence)
{
	UGenAISettings* Settings = UGenAISettings::Get();
	if (!Settings) return;
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: ApplyAdherence %.2f, Mode=%d"), Adherence, static_cast<int32>(Settings->GenerationMode));
...
void SUE_GenPanel::ApplySettingsToConfig()
{
	UGenAISettings* Settings = UGenAISettings::Get();
	if (Settings)
	{
...
void SUE_GenPanel::OnGenerationComplete(bool bSuccess, UTexture2D* ResultTexture)
{
	...
	if (CurrentStageIndex >= 0 && CurrentStageIndex < TotalStages - 1 && bSuccess)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen Sequence: Stage %d/%d complete, advancing..."),
	...
	if (bSuccess && ResultTexture)
	{
		...
		if (GraphEditor.IsValid())
		{
			for (const FGraphNode& Node : GraphEditor->GetNodes())
			{
				...
				{
					UE_LOG(LogTemp, Log, TEXT("UE_Gen: Image Upres node detected, triggering UE-side export"));
		...
		if (HttpClient.IsValid() && HttpClient->Has3DModelResult())
		{
			...
			{
				UE_LOG(LogTemp, Log, TEXT("UE_Gen: 3D model detected alongside image, triggering 3D Asset Export"));
				Execute3DAssetExportNodes();
			}
			else if (bHasSave3DModelNode)
			{
				UE_LOG(LogTemp, Log, TEXT("UE_Gen: 3D model also detected alongside image, triggering Save3DModel import"));
	...
	else if (bSuccess && !ResultTexture)
	{
		...
		if (bHas3DAssetExportNode && HttpClient.IsValid() && HttpClient->Has3DModelResult())
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: 3D model detected in workflow output, triggering 3D Asset Export"));
			Execute3DAssetExportNodes();
		}
		else if (bHasSave3DModelNode && HttpClient.IsValid() && HttpClient->Has3DModelResult())
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: 3D model detected in workflow output, triggering Save3DModel import"));
			ExecuteSave3DModelNodes();
		}
		else if (bHasMeshyNode)
		{
			UpdateStatusText(TEXT("Meshy: 3D model generation complete! Processing..."));
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Meshy workflow completed, triggering import flow"));
			ExecuteMeshyImportNodes();
		}
		else if (bHasSave3DModelNode || bHas3DAssetExportNode)
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Graph has 3D model node but no 3D model was detected in workflow output. "
		...
		else
		{
			...
			if (bIsVideoResult)
			{
				UE_LOG(LogTemp, Log, TEXT("UE_Gen: Video result detected: %s — downloading and extracting thumbnail"),
...
void SUE_GenPanel::OnGenerationProgress(float Progress)
{
...
void SUE_GenPanel::OnGenerationError(const FString& ErrorMessage)
{
	bIsGenerating = false;
	bIsGeneratingVideo = false;
	CurrentProgress = 0.0f;
	...
	UE_LOG(LogTemp, Error, TEXT("UE_Gen: %s"), *ErrorMessage);
}
...
void SUE_GenPanel::UpdateThumbnailBrush(TSharedPtr<FSlateBrush>& Brush,
...
void SUE_GenPanel::UpdateStatusText(const FString& Message)
{
...
TArray<FLoRAEntry> SUE_GenPanel::GetActiveLoRAs() const
{
...
FString SUE_GenPanel::EstimateGenerationCost() const
{
	const UGenAISettings* Settings = UGenAISettings::Get();
...
FString SUE_GenPanel::EstimateVideoCost() const
{
	const UGenAISettings* Settings = UGenAISettings::Get();
	if (!Settings->bShowCostEstimates)
	{
...
TSharedRef<SWidget> SUE_GenPanel::BuildWorkflowPreviewSection()
{
	return SNew(SVerticalBox)
...
void SUE_GenPanel::RefreshWorkflowPreview()
{
...
TSharedRef<SWidget> SUE_GenPanel::BuildGraphEditorSection()
{
...
void SUE_GenPanel::OnGraphSelectionChanged()
{
	RebuildNodeDetailsPanel();
}

void SUE_GenPanel::RebuildNodeDetailsPanel()
{
			...
			[
				...
				[
					...
					.OnClicked_Lambda([CapturedPath]() -> FReply
					{
						FString NativePath = CapturedPath;
						FPaths::MakePlatformFilename(NativePath);
						UE_LOG(LogTemp, Log, TEXT("UE_Gen: Play Video clicked — launching: %s"), *NativePath);
						FPlatformProcess::LaunchFileInDefaultExternalApplication(*NativePath);
						return FReply::Handled();
					})
				]
				...
				[
					...
					.OnClicked_Lambda([CapturedPath]() -> FReply
					{
						...
						UE_LOG(LogTemp, Log, TEXT("UE_Gen: Open Folder clicked — exploring: %s"), *FolderPath);
						FPlatformProcess::ExploreFolder(*NativePath);
						return FReply::Handled();
					})
				]
			...
			[
				...
				[
					...
					.OnClicked_Lambda([this, CapturedNodeId, CapturedServerFilename]() -> FReply
					{
						...
						FString VideoDir = FPaths::ProjectSavedDir() / TEXT("UE_Gen") / TEXT("VideoCache");
						IFileManager::Get().MakeDirectory(*VideoDir, true);
						FString LocalPath = VideoDir / CapturedServerFilename;

						UE_LOG(LogTemp, Log, TEXT("UE_Gen: Downloading video '%s' from ComfyUI for playback..."), *CapturedServerFilename);
							...
							[this, CapturedNodeId, LocalPath, CapturedServerFilename](const FString& DownloadedPath)
						{
							...
							{
								UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to download video '%s' from ComfyUI"), *CapturedServerFilename);
							...
							UE_LOG(LogTemp, Log, TEXT("UE_Gen: Downloaded video, launching: %s"), *NativeDLPath);
...
void SUE_GenPanel::RefreshLoadImageThumbnails()
{
...
void SUE_GenPanel::RefreshLoadVideoThumbnails()
{
...
void SUE_GenPanel::OnBrowseNodeImage(FString NodeId, FString WidgetName)
{
...
void SUE_GenPanel::OnBrowseNodeVideo(FString NodeId, FString WidgetName)
{
	IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get();
	if (!DesktopPlatform) return;
...
void SUE_GenPanel::OnQuickRenderForNode(FString NodeId)
{
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Starting Quick Render for sequence '%s', output to '%s'"),
	...
	if (GEditor)
	{
		GEditor->GetTimerManager()->SetTimer(QuickRenderPollTimer, FTimerDelegate::CreateRaw(this, &SUE_GenPanel::PollQuickRenderOutput),
			1.0f, true); // Poll every 1 second
	}
}

void SUE_GenPanel::PollQuickRenderOutput()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Quick Render timed out after 10 minutes"));
	...
	if (NewestVideoPath == LastSeenPath && CurrentSize == LastSeenSize && CurrentSize > 0)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Quick Render complete, output: %s (%s)"),
...
void SUE_GenPanel::ExtractVideoThumbnail(const FString& VideoFilePath, const FString& NodeId)
{
	if (!GraphEditor.IsValid()) return;
	...
	FString TempDir = FPaths::ProjectSavedDir() / TEXT("UE_Gen") / TEXT("VideoThumbs");
	...
	{
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: ffmpeg not available or failed to extract video thumbnail (return code %d). "
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to read extracted thumbnail: %s"), *TempPNG);
		return;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to decode video thumbnail PNG"));
		return;
	}
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to decompress video thumbnail pixels"));
		return;
	}
	...
	if (ThumbTex)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Set video thumbnail for node %s (%dx%d)"), *NodeId, ImgWidth, ImgHeight);
	}
...
void SUE_GenPanel::HandleVideoResult()
{
	if (!HttpClient.IsValid()) return;
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: HandleVideoResult called but no result filename available"));
		return;
	}

	// Build local save path
	FString VideoDir = FPaths::ProjectSavedDir() / TEXT("UE_Gen") / TEXT("VideoResults");
	IFileManager::Get().MakeDirectory(*VideoDir, true);
		...
		[this, CapturedLocalPath, CapturedFilename](const FString& DownloadedPath)
	{
		if (DownloadedPath.IsEmpty())
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to download video result: %s"), *CapturedFilename);
			UpdateStatusText(TEXT("Video generation complete (download failed — check Output Log)"));
			return;
		}

		LastVideoResultPath = DownloadedPath;
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Video downloaded to: %s"), *DownloadedPath);
		...
		FString ThumbDir = FPaths::ProjectSavedDir() / TEXT("UE_Gen") / TEXT("VideoThumbs");
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: ffmpeg failed to extract video thumbnail (code %d). "
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Video result added to history with thumbnail (%dx%d): %s"),
			ImgWidth, ImgHeight, *DownloadedPath);
	});
}

void SUE_GenPanel::OnBrowse3DModelFile(FString NodeId)
{
	IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get();
	if (!DesktopPlatform) return;
	...
	FString AssetPath = TEXT("/Game/UE_Gen/Models");
	FString AssetName;
...
void SUE_GenPanel::OnBrowseAssetPath(FString NodeId, FString WidgetName)
{
	IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get();
	...
	else
	{
		...
		else
		{
			...
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Selected folder '%s' is outside the Content directory"), *SelectedFolder);
...
void SUE_GenPanel::OnBrowseDiskFolder(FString NodeId, FString WidgetName)
{
	IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get();
	if (!DesktopPlatform || !GraphEditor.IsValid()) return;
...
void SUE_GenPanel::OnPromptTextChanged(const FText& NewText)
{
...
void SUE_GenPanel::OnNegativePromptTextChanged(const FText& NewText)
{
	if (bAutoSyncPrompts && !bSyncInProgress)
	{
		PerformPromptSync();
...
void SUE_GenPanel::PerformPromptSync()
{
...
FReply SUE_GenPanel::OnGenerateFromGraphClicked()
{
	...
	if (GraphEditor->HasSequenceNode())
	{
		...
		for (const FGraphNode& VNode : GraphEditor->GetNodes())
		{
			...
			FString TempDir = FPaths::ProjectSavedDir() / TEXT("UE_Gen") / TEXT("VideoFrames");
			IFileManager::Get().MakeDirectory(*TempDir, true);
	...
	if (bNeedsSegmentation && SegmentationCapture.IsValid())
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Segmentation mask uploaded (%d segments found)"),
	...
	if (GraphEditor.IsValid())
	{
		for (const FGraphNode& VNode : GraphEditor->GetNodes())
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen: Video to Image node '%s': extracted frame %d from '%s' → uploaded as '%s'"),
	...
	{
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Meshy node detected, source image: %s"), *MeshySourceImage);
	...
	if (!bWorkflowHasComfyUIWork && !PendingMeshyImageFilename.IsEmpty())
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: No ComfyUI processing needed, triggering Meshy directly"));
		bIsGenerating = true;
	...
	if (!bWorkflowHasComfyUIWork && !PendingMeshyImageBase64.IsEmpty())
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: No ComfyUI processing needed, triggering Meshy directly from viewport"));
		bIsGenerating = true;
		UpdateStatusText(TEXT("Starting Meshy 3D generation..."));
		OnGenerationComplete(true, nullptr);
		return FReply::Handled();
...
void SUE_GenPanel::SubmitNextStagedWorkflow()
{
	CurrentStageIndex++;
	...
	if (!StageWorkflow.IsValid() || StageWorkflow->Values.Num() == 0)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen Sequence: Stage %d is empty, skipping"), CurrentStageIndex + 1);
		SubmitNextStagedWorkflow();
		return;
	}
...
void SUE_GenPanel::ResolveUEImageMarker(TSharedPtr<FJsonObject> Workflow, const FString& Marker, const FString& ServerFilename)
{
	for (const auto& Pair : Workflow->Values)
	{
		TSharedPtr<FJsonObject> NodeObj = Pair.Value->AsObject();
...
void SUE_GenPanel::ResolveUECameraData(TSharedPtr<FJsonObject> Workflow, const FString& CameraDescription)
{
...
FString SUE_GenPanel::GetWorkflowsDirectory() const
{
	// Try the project Plugins directory first
	FString WorkflowDir = FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("UE_Gen"), TEXT("Workflows"));

	// If the plugin isn't in the project Plugins dir, try the engine Plugins dir
	if (!IFileManager::Get().DirectoryExists(*FPaths::Combine(FPaths::ProjectPluginsDir(), TEXT("UE_Gen"))))
	{
		// Fallback to Saved/UE_Gen/Workflows in the project directory
		WorkflowDir = FPaths::Combine(FPaths::ProjectSavedDir(), TEXT("UE_Gen"), TEXT("Workflows"));
	}
...
FReply SUE_GenPanel::OnSaveGraphClicked()
{
...
FReply SUE_GenPanel::OnSaveGraphAsClicked()
{
	if (!GraphEditor.IsValid()) return FReply::Handled();
...
FReply SUE_GenPanel::OnLoadGraphClicked()
{
	if (!GraphEditor.IsValid()) return FReply::Handled();
...
FReply SUE_GenPanel::OnGraphEditorFullscreen()
{
	if (!GraphEditor.IsValid()) return FReply::Handled();
	...
	TSharedRef<SWindow> NewWindow = SNew(SWindow)
		.Title(FText::FromString(TEXT("UE Gen - Graph Editor")))
	...
	TWeakPtr<SUE_GenPanel> WeakPanel = SharedThis(this);
	NewWindow->SetOnWindowClosed(FOnWindowClosed::CreateLambda(
		[WeakPanel](const TSharedRef<SWindow>&)
		{
			TSharedPtr<SUE_GenPanel> Panel = WeakPanel.Pin();
			if (!Panel.IsValid())
			{
...
void SUE_GenPanel::CloseGraphEditorFullscreen()
{
	if (GraphEditorFullscreenWindow.IsValid())
	{
...
TSharedRef<SWidget> SUE_GenPanel::BuildVideoTab()
{
				...
				[
					...
					[
						...
						[
							...
							.OnClicked(this, &SUE_GenPanel::OnLoadSourceFrameClicked)
							[
								SNew(STextBlock)
		...
		[
			...
			[
				...
				[
					...
					.OnClicked(this, &SUE_GenPanel::OnAnimateClicked)
					[
						SNew(STextBlock)
...
TSharedRef<SWidget> SUE_GenPanel::BuildVideoSettingsPanel()
{
...
void SUE_GenPanel::ApplyVideoAdherenceToSettings(float Adherence)
{
	UGenAISettings* Settings = UGenAISettings::Get();
...
void SUE_GenPanel::SetVideoSourceFromPreview()
{
...
void SUE_GenPanel::SetVideoSourceFromTexture(UTexture2D* Tex)
{
...
FReply SUE_GenPanel::OnLoadSourceFrameClicked()
{
	IDesktopPlatform* DesktopPlatform = FDesktopPlatformModule::Get();
	if (!DesktopPlatform) return FReply::Handled();
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to load source frame from: %s"), *FilePath);
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Image decode failed for: %s"), *FilePath);
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to get raw pixel data from: %s"), *FilePath);
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Loaded source frame from: %s (%dx%d)"), *FilePath, Width, Height);
	UpdateStatusText(FString::Printf(TEXT("Loaded source frame: %s (%dx%d)"), *FPaths::GetCleanFilename(FilePath), Width, Height));

	return FReply::Handled();
}

FString SUE_GenPanel::TextureToBase64PNG(UTexture2D* Texture)
{
	...
	if (!MipData)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: TextureToBase64PNG — could not lock texture data"));
		return FString();
	}
	...
	if (PNGData.Num() == 0)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: TextureToBase64PNG — PNG compression failed"));
		return FString();
	}
...
FReply SUE_GenPanel::OnAnimateClicked()
{
	...
	if (GEditor)
	{
		GEditor->GetTimerManager()->SetTimer(
			ProgressPollTimer,
			FTimerDelegate::CreateRaw(this, &SUE_GenPanel::PollVideoProgress),
...
void SUE_GenPanel::PollVideoProgress()
{
	if (HttpClient.IsValid())
	{
		HttpClient->PollProgress();
...
void SUE_GenPanel::ShowPreviousResult()
{
	if (HistoryIndex > 0)
	{
...
void SUE_GenPanel::ShowNextResult()
{
...
void SUE_GenPanel::ShowHistoryEntry(int32 Index)
{
...
TSharedRef<SWidget> SUE_GenPanel::BuildResultGalleryPanel()
{
	const UGenAISettings* Settings = UGenAISettings::Get();
		...
		[
			...
			[
				...
				[
					...
					.OnClicked(this, &SUE_GenPanel::OnSegmentWithSAM3Clicked)
				]
			]
			...
			[
				...
				.OnClicked(this, &SUE_GenPanel::OnConvertAllSelectedClicked)
			]
#endif
		];
}

void SUE_GenPanel::RebuildResultGallery()
{
...
void SUE_GenPanel::PopulateSegmentationGallery()
{
...
FReply SUE_GenPanel::OnSegmentWithSAM3Clicked()
{
...
void SUE_GenPanel::OnSAM3SegmentationComplete(
	...
	UTexture2D* OriginalTexture)
{
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: SAM3 returned %d masks + visualization"), MaskTextures.Num());
...
void SUE_GenPanel::ApplyMasksToOriginal(UTexture2D* OriginalTexture, const TArray<UTexture2D*>& MaskTextures)
{
	ClearSAM3Segments();
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen: Created %d isolated segment images"), SAM3Segments.Num());
...
void SUE_GenPanel::RebuildSegmentationGallery()
{
...
void SUE_GenPanel::ClearSAM3Segments()
{
	for (FSAM3Segment& Seg : SAM3Segments)
	{
...
void SUE_GenPanel::ConvertSegmentToMesh(int32 SegmentIndex)
{
...
FReply SUE_GenPanel::OnConvertAllSelectedClicked()
{
...
void SUE_GenPanel::OnMeshyTaskCreated(bool bSuccess, const FString& TaskIdOrError, int32 SegmentIndex)
{
	if (!bSuccess)
	{
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Meshy: Task created for segment %d: %s"), SegmentIndex, *TaskId);
	...
	{
		GEditor->GetTimerManager()->SetTimer(MeshyPollTimer, FTimerDelegate::CreateRaw(this, &SUE_GenPanel::PollMeshyTasks),
...
void SUE_GenPanel::PollMeshyTasks()
{
...
void SUE_GenPanel::OnMeshyTaskProgress(const FMeshyTaskResult& Result)
{
	for (FSAM3Segment& Seg : SAM3Segments)
	{
...
void SUE_GenPanel::OnMeshyTaskComplete(const FMeshyTaskResult& Result)
{
	for (FSAM3Segment& Seg : SAM3Segments)
	{
		if (Seg.MeshyTaskId == Result.TaskId)
	...
	if (Result.Status == TEXT("SUCCEEDED") && !Result.GLBUrl.IsEmpty())
	{
		...
		MeshyClient->OnModelDownloaded.BindRaw(this, &SUE_GenPanel::OnMeshyModelDownloaded);
...
void SUE_GenPanel::OnMeshyModelDownloaded(const FString& LocalFilePath)
{
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Meshy: GLB downloaded to %s"), *LocalFilePath);
...
void SUE_GenPanel::ExecuteSave3DModelNodes()
{
	if (!GraphEditor.IsValid() || !HttpClient.IsValid()) return;
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Save3DModel: No 3D model detected in workflow output"));
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Save3DModel: Downloading '%s' from ComfyUI (subfolder=%s, type=%s)"),
		 ...
		 CapturedModelFilename, CapturedModelSubfolder, CapturedLocalFilePath, ModelFolderType](const FString& DownloadedPath)
		{
			...
			if (DownloadedPath.IsEmpty())
			{
				...
				if (AlternateTypes.Num() > 0 && HttpClient.IsValid())
				{
					...
					UE_LOG(LogTemp, Log, TEXT("UE_Gen Save3DModel: Retrying download with type='%s'"), *RetryType);
					...
					HttpClient->DownloadComfyUIFile(CapturedModelFilename, CapturedModelSubfolder, RetryType, CapturedLocalFilePath,
						 ...
						 CapturedModelFilename, CapturedModelSubfolder, CapturedLocalFilePath, AlternateTypes](const FString& RetryPath)
						{
							if (RetryPath.IsEmpty() && AlternateTypes.Num() > 1 && HttpClient.IsValid())
							{
								...
								UE_LOG(LogTemp, Log, TEXT("UE_Gen Save3DModel: Retrying download with type='%s'"), *RetryType2);
...
void SUE_GenPanel::Handle3DModelImport(const FString& DownloadedPath, const FString& AssetPath, const FString& AssetName, bool bPlaceInLevel)
{
	UE_LOG(LogTemp, Log, TEXT("UE_Gen Save3DModel: Model downloaded to %s"), *DownloadedPath);
...
void SUE_GenPanel::Execute3DAssetExportNodes()
{
	if (!GraphEditor.IsValid() || !HttpClient.IsValid()) return;

	if (!HttpClient->Has3DModelResult())
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen 3DAssetExport: No 3D model detected in workflow output"));
		UpdateStatusText(TEXT("No 3D model found in workflow output"));
		return;
	}
	...
	UpdateStatusText(FString::Printf(TEXT("Downloading 3D model: %s"), *ModelFilename));
	UE_LOG(LogTemp, Log, TEXT("UE_Gen 3DAssetExport: Downloading '%s' from ComfyUI (subfolder=%s, type=%s)"),
		*ModelFilename, *ModelSubfolder, *ModelFolderType);
	...
	HttpClient->DownloadComfyUIFile(ModelFilename, ModelSubfolder, ModelFolderType, LocalFilePath,
		 ...
		 CapturedModelFilename, CapturedModelSubfolder, CapturedLocalFilePath, ModelFolderType](const FString& DownloadedPath)
		{
			if (DownloadedPath.IsEmpty())
			{
				...
				{
					FString RetryType = AlternateTypes[0];
					UE_LOG(LogTemp, Log, TEXT("UE_Gen 3DAssetExport: Retrying download with type='%s'"), *RetryType);
...
void SUE_GenPanel::Handle3DAssetExport(const FString& DownloadedPath, const FString& AssetPath,
	const FString& AssetName, const FString& CollisionMode, bool bEnableNanite, bool bPlaceInLevel)
{
	UE_LOG(LogTemp, Log, TEXT("UE_Gen 3DAssetExport: Model downloaded to %s"), *DownloadedPath);
		...
		[this, DownloadedPath, AssetPath, AssetName, CollisionMode, bEnableNanite, bPlaceInLevel](float) -> bool
	{
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen 3DAssetExport: Could not load imported mesh at '%s' for post-import settings"), *ImportedAssetPath);
		}
...
void SUE_GenPanel::ExecuteImageUpresExport()
{
	if (!GraphEditor.IsValid()) return;
	...
	FString SavePath;
	FString FilenamePrefix = TEXT("UE_Gen_Upres");
	...
	if (SavePath.IsEmpty())
	{
		SavePath = FPaths::ProjectSavedDir() / TEXT("UE_Gen_Upres");
	}

	// Get the result texture from the latest history entry
	if (ImageHistory.Num() == 0 || HistoryIndex < 0 || HistoryIndex >= ImageHistory.Num())
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen ImageUpres: No result image available for export"));
	...
	if (!SourceTexture)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen ImageUpres: Result texture is null"));
	...
	UE_LOG(LogTemp, Log, TEXT("UE_Gen ImageUpres: Exporting %dx%d (%s %s-bit) from history texture"),
	...
	if (!PixelData)
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen ImageUpres: Failed to lock texture bulk data for reading"));
	...
	if (OutputFormat == TEXT("EXR"))
	{
		...
		if (BitDepthInt == 16)
		{
			...
			if (!bSetOk)
			{
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen ImageUpres: EXR SetRaw failed (16-bit RGBAF, %dx%d, %lld bytes)"),
		...
		else
		{
			...
			if (!bSetOk)
			{
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen ImageUpres: EXR SetRaw failed (32-bit RGBAF, %dx%d, %lld bytes)"),
		...
		if (CompressedData.Num() > 0)
		{
			...
			{
				UE_LOG(LogTemp, Log, TEXT("UE_Gen ImageUpres: Saved %d-bit EXR (%dx%d) to %s"),
			...
			{
				UE_LOG(LogTemp, Warning, TEXT("UE_Gen ImageUpres: Failed to write file: %s"), *FullOutputPath);
				UpdateStatusText(TEXT("Image Upres: Failed to save EXR file"));
			}
		}
		else
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen ImageUpres: EXR compression returned empty data"));
	...
	else
	{
		...
		if (CompressedData.Num() > 0)
		{
			...
			if (FFileHelper::SaveArrayToFile(FileBytes, *FullOutputPath))
			{
				UE_LOG(LogTemp, Log, TEXT("UE_Gen ImageUpres: Saved PNG (%dx%d) to %s"), Width, Height, *FullOutputPath);
		...
		{
			UE_LOG(LogTemp, Warning, TEXT("UE_Gen ImageUpres: PNG compression returned empty data"));
			UpdateStatusText(TEXT("Image Upres: PNG encoding failed"));
		}
	}
}

void SUE_GenPanel::Apply3DAssetExportSettings(UStaticMesh* Mesh, const FString& CollisionMode, bool bEnableNanite)
{
	if (!Mesh) return;

	UE_LOG(LogTemp, Log, TEXT("UE_Gen 3DAssetExport: Applying settings — collision='%s', nanite=%s"),
	...
	if (bEnableNanite)
	{
		...
		if (!CurrentNaniteSettings.bEnabled)
		{
			...
			bModified = true;
			UE_LOG(LogTemp, Log, TEXT("UE_Gen 3DAssetExport: Nanite enabled for mesh"));
		}
		else
		{
			UE_LOG(LogTemp, Log, TEXT("UE_Gen 3DAssetExport: Nanite already enabled by importer, skipping"));
		}
	}
#endif
...
void SUE_GenPanel::ExecuteMeshyImportNodes()
{
	if (!GraphEditor.IsValid()) return;

	// Collect the first __UE_MeshyImport__ node's settings
	FString ModelFormat = TEXT("GLB");
	FString AssetPath = TEXT("/Game/UE_Gen/MeshyModels");
	FString AssetName;
	bool bPlaceInLevel = true;
	...
	if (!bFoundImportNode)
	{
		UE_LOG(LogTemp, Log, TEXT("UE_Gen MeshyImport: No __UE_MeshyImport__ node found, using defaults"));
	}
	...
	for (const FSAM3Segment& Seg : SAM3Segments)
	{
		...
		{
			UE_LOG(LogTemp, Log, TEXT("UE_Gen MeshyImport: Using SAM3 segment task %s"), *Seg.MeshyTaskId);
	...
	{
		UpdateStatusText(TEXT("Meshy Import: Meshy API key is not set. Configure it in Project Settings > Plugins > UE Gen."));
		return;
	}
	...
	if (!PendingMeshyImageBase64.IsEmpty())
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen MeshyImport: Using pending viewport image for Meshy API call"));
	...
	else if (!PendingMeshyImageFilename.IsEmpty())
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen MeshyImport: Downloading '%s' from ComfyUI for Meshy API call"), *PendingMeshyImageFilename);
		...
		HttpClient->DownloadComfyUIImageAsBase64(CapturedFilename, [this, CapturedFormat, CapturedAssetPath, CapturedAssetName, bCapturedPlace](const FString& Base64PNG)
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen MeshyImport: Image downloaded, launching Meshy task"));
	...
	else
	{
		...
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen MeshyImport: No pending image and no SAM3 segments"));
	}
}

void SUE_GenPanel::FetchAndImportMeshyTask(const FString& TaskId, const FString& ModelFormat,
...
void SUE_GenPanel::LaunchMeshyTaskFromImage(const FString& Base64PNG, const FString& ModelFormat,
	const FString& AssetPath, const FString& AssetName, bool bPlaceInLevel)
{
	const UGenAISettings* Settings = UGenAISettings::Get();

	UpdateStatusText(TEXT("Meshy: Creating Image-to-3D task..."));
	UE_LOG(LogTemp, Log, TEXT("UE_Gen MeshyImport: Launching Meshy task (image size: %d chars base64)"), Base64PNG.Len());
	...
	TaskClient->OnTaskCreated.BindLambda([this, CapturedFormat, CapturedAssetPath, CapturedAssetName, bCapturedPlace, TaskClient](bool bSuccess, const FString& TaskIdOrError)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen MeshyImport: Task created: %s — polling for completion..."), *TaskIdOrError);
...
void SUE_GenPanel::ImportMeshyModelToLevel(const FMeshyTaskResult& TaskResult, const FString& ModelFormat,
	const FString& AssetPath, const FString& AssetName, bool bPlaceInLevel)
{
	...
	DownloadClient->OnModelDownloaded.BindLambda([this, CapturedAssetPath, CapturedAssetName, bCapturedPlace, DownloadClient](const FString& DownloadedPath)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen MeshyImport: Model downloaded to %s"), *DownloadedPath);
...
FString SUE_GenPanel::ImportModelFileToContentBrowser(const FString& LocalFilePath, const FString& DestAssetPath, const FString& DesiredAssetName)
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen Import: AssetTools.ImportAssets returned no results for %s"), *LocalFilePath);
		return FString();
	}
	...
	for (UObject* Asset : ImportedAssets)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen Import: Imported asset [%s]: %s"),
	...
	if (!FoundMesh)
	{
		...
		for (const FAssetData& AssetData : FoundAssets)
		{
			...
			if (ClassName == TEXT("StaticMesh"))
			{
				...
				if (FoundMesh)
				{
					UE_LOG(LogTemp, Log, TEXT("UE_Gen Import: Found StaticMesh via registry: %s"), *AssetData.GetObjectPathString());
					break;
				}
			}
		}
	...
	if (FoundMesh)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen Import: Using StaticMesh: %s"), *MeshPath);
	...
	if (FallbackObj)
	{
		...
		UE_LOG(LogTemp, Log, TEXT("UE_Gen Import: No StaticMesh found, using fallback: %s"), *ImportedPath);
		return ImportedPath;
	}

	UE_LOG(LogTemp, Warning, TEXT("UE_Gen Import: No valid assets imported from %s"), *LocalFilePath);
	return FString();
}

AActor* SUE_GenPanel::PlaceAssetInLevel(const FString& AssetContentPath)
{
	if (!GEditor || !GEditor->GetEditorWorldContext().World())
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen MeshyImport: No editor world available for placement"));
		return nullptr;
	}
	...
	if (StaticMesh)
	{
		...
		if (MeshActor && MeshActor->GetStaticMeshComponent())
		{
			...
			UE_LOG(LogTemp, Log, TEXT("UE_Gen MeshyImport: Placed StaticMeshActor '%s' in level at origin"), *MeshActor->GetName());
			return MeshActor;
		}
	}

	UE_LOG(LogTemp, Warning, TEXT("UE_Gen MeshyImport: Could not load or place asset at path: %s"), *AssetContentPath);
	return nullptr;
}
...
FString SUE_GenPanel::GetAutoSaveGraphPath()
{
	return FPaths::ProjectSavedDir() / TEXT("UE_Gen") / TEXT("LastGraph.json");
}

void SUE_GenPanel::AutoSaveGraph()
{
	if (!GraphEditor.IsValid()) return;
...
bool SUE_GenPanel::RestoreLastGraph()
{
	...
	{
		UE_LOG(LogTemp, Warning, TEXT("UE_Gen: Failed to parse auto-save graph file"));
		return false;
	}

	if (GraphEditor->DeserializeGraph(GraphJson))
	{
		UE_LOG(LogTemp, Log, TEXT("UE_Gen: Restored last-edited graph from auto-save"));
		RefreshLoadImageThumbnails();
		return true;
	}
